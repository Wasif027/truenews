...
# --- bring-your-own-key: a visitor's own key, for one request, off the pool ---

class BYOKError(RuntimeError):
    """The visitor's own API key failed — surface this plainly rather than
    quietly falling back to a shared-pool result that has nothing to do with
    the key they gave us."""


# Endpoint + model per provider so a visitor only has to paste a raw key, not
# configure an endpoint. Mirrors the LLM_1 (Gemini) / LLM_2 (Groq) defaults in
# ingest.yml — same free-tier models, just billed against their own account.
_BYOK_PRESETS: dict[str, tuple[str, str]] = {
    "gemini": (
...
def call_llm_json_with(
    provider: str, api_key: str, system: str, user: str, *, timeout: float = 50.0
) -> dict:
    """Same JSON-mode call as `call_llm_json`, but against one visitor-supplied
    key — no shared rate-limit gap, no shared cooldown (it isn't the shared
    pool), and a single pass over that provider's model list. `api_key` is used
    only for the request(s) made here; never logged, never written anywhere."""
    preset = _BYOK_PRESETS.get(provider)
    if not preset or not api_key.strip():
        raise BYOKError(f"unsupported provider {provider!r}")
    base, model_csv = preset
    key = api_key.strip()

    last_error = "the model didn't return a usable response"
    for model in model_csv.split(","):
        model = model.strip()
        try:
            resp = httpx.post(
                f"{base}/chat/completions",
        ...
        if resp.status_code in (401, 403) or (
            ...
            raise BYOKError("that API key was rejected by the provider")
        if resp.status_code != 200:
            last_error = f"the provider returned {resp.status_code}"
            continue
        try:
            data = json.loads(resp.json()["choices"][0]["message"]["content"])
            if isinstance(data, dict):
            ...
            last_error = "the model's response wasn't valid JSON"
    raise BYOKError(last_error)
